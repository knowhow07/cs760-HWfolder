...
if __name__ == "__main__":
    ...
    def split_train_test(data,n_samples):
        n_data = data.shape[0]
        ...
        train_feats = data[:n_samples, :2]
        # slice n_samples of only the last column as trgts
        train_trgts = data[:n_samples, -1:]
        # slice n_samples up until the last column as feats
        test_feats = data[n_samples:n_data, :2]
        # slice n_samples of only the last column as trgts
        test_trgts = data[n_samples:n_data, -1:]
    ...
    #number of n_samples
    n_samples = 32
    train, test = split_train_test(data_ran,n_samples)
    X_train = train[:,:2]
    y_train = np.asarray(train[:,2:3].flatten(),dtype='int')
    ...
    fig, ax = plt.subplots()
# title for the plots
    title = ('Decision boundary of decision tree ')
